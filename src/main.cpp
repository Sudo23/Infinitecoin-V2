...
unsigned int nStakeMinAge = 60 * 60 * 24 * 15;	// minimum age for coin age: 7d
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;	// stake age of full weight: 30d
unsigned int nStakeTargetSpacing = 25;			// 30 sec block spacing

int64 nChainStartTime = 1377186039;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ColossusCoin
    bool fStrictPayToScriptHash = true; // Always active in ColossusCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ColossusCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Infinite Coin V2 LOL";
        CTransaction txNew;
		...
		txNew.strTxComment = "text: Infinitecoinv2 genesis block";
   ...
   if (false  && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x66bb3fe4b8024dd1c8a0052fed1106e6abde6a6d98b897543266578ccd219ca8"));
...
string GetWarnings(string strFor)
{
    ...
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 40) && !fTestNet && !IsInitialBlockDownload())
    {
        nPriority = 100;
